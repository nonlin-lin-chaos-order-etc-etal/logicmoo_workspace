...
% ensure this file does not get unloaded with pfc_reset
...
%
%:- kb_shared(t/1).
%t([P|LIST]):- cwc, !,pfc_plist_t(P,LIST).
%t(naf(CALL)):- cwc, !,not(t(CALL)).
%t(not(CALL)):- cwc, !,pfc_f(CALL).

% BEST t(CALL):- cwc, call(into_plist_arities(3,10,CALL,[P|LIST])),pfc_plist_t(P,LIST).
...
t(P,A1):- cwc, pfc_fa_call(P,1,call(P,A1)).
...
% True Structure.
%
t(P,A1,A2):- cwc,  pfc_fa_call(P,2,call(P,A1,A2)).
...
% True Structure.
%
t(P,A1,A2,A3):- cwc,  pfc_fa_call(P,3,call(P,A1,A2,A3)).
...
% True Structure.
%
t(P,A1,A2,A3,A4):- cwc,  pfc_fa_call(P,4,call(P,A1,A2,A3,A4)).
...
% True Structure.
%
t(P,A1,A2,A3,A4,A5):- cwc,  pfc_fa_call(P,5,call(P,A1,A2,A3,A4,A5)).
...
% True Structure.
%
t(P,A1,A2,A3,A4,A5,A6):- cwc,  pfc_fa_call(P,6,call(P,A1,A2,A3,A4,A5,A6)).
...
% True Structure.
%
t(P,A1,A2,A3,A4,A5,A6,A7):- cwc,  pfc_fa_call(P,7,call(P,A1,A2,A3,A4,A5,A6,A7)).
...
rtArgsVerbatum( <- ).
rtArgsVerbatum(=..).
% rtArgsVerbatum({}). % Needs pfc_expansion to visit
...
:- (pfc_ain(==>pfcControlled(pfc_undo_sys(ftAssertion, ftCallable, ftCallable)))).
:- (ain(==>pfcControlled(pfc_undo_sys(ftAssertion, ftCallable, ftCallable)))).
pfc_undo_sys(P, WhenAdded, WhenRemoved) ==> (P ==> {WhenAdded}), pfc_do_and_undo_method(WhenAdded,WhenRemoved).

% DONT pfc_undo_sys(added(P),ain(P),pfc_retract(P)).
...
%:- rtrace,dtrace.
% ==>(prologBuiltin(pfc_select_hook/1)).
% :- nortrace,quietly.

:- kb_shared(conflict/1).
% a conflict triggers a Prolog action to resolve it.
conflict(C) ==> {must(with_pfc_trace_exec((resolveConflict(C),\+conflict(C))))}.


% meta rules to schedule inferencing.
% resolve conflicts asap
%baseKB:pfc_select_hook(conflict(X)) :- cwc, que(conflict(X),_Why).
...
never_assert_u(pfc_unload_option(never,X),is_ftVar(pfc_unload_option(never,X))):- cwc, is_ftVar(X).
never_assert_u(X,is_ftVar(X)):- cwc, is_ftVar(X).
...
%:- dynamic(pfc_unload_option/2).
%:- listing(pfc_unload_option/2).
%:- pfc_trace_exec.
%:- rtrace,trace.
:- prolog_load_context(file,F), ain(pfc_unload_option(F,never)).
%:- nortrace.
%:- pfc_notrace_exec.
%:- listing(pfc_unload_option/2).

%:- pfc_trace_exec.
%(P/pfc_positive_fact(P),~P) ==> \+ ~P.
...
preventedWhen(P,{Cond})/pfc_positive_fact(P)==> ((~P) :- cwc, Cond).

%:- pfc_trace_exec.
%  can this ever happen?
% (( \+ P, P) ==> {dumpST,dmsg_pretty(warn(weak_conflict(P)))}).
% TAKEN CARE OF ( (~ P/pfc_positive_fact(P)), P) ==> ({dmsg_pretty(warn(conflict(P)))}).
% (\+ P, P) => conflict(P).
...
% BWD chaining
mdefault((Q <- P))/pfc_literal(Q) ==> (Q <-(P, \+ ~(Q))).

% FWD chaining
mdefault(P==>Q)/nonvar(Q) ==> (((P ==> mdefault(Q)))).

% NEG chaining
mdefault(~Q)/pfc_positive_fact(Q)  ==>  (( \+ Q ) ==> ~ Q ).


% POS chaining 1 (Neg Trigger)
mdefault(Q)/(pfc_positive_fact(Q),if_missing_mask(Q,R,Test)) ==> (  ( ( \+R /(ground(R),Test), (\+ ~Q )) ==> Q )).

% POS chaining 2 (Pos Trigger)
mdefault(Q)/(pfc_positive_fact(Q),if_missing_mask(Q,R,Test)) ==> ( ((R/(ground(R), Test, \+(R=Q))) ==> (\+ Q))).


% mdefault(Q) ==> if_missing(Q,Q).
%(mdefault(P=>Q)/(pfc_literal_nv(Q),if_missing_mask(Q,R,Test)))  ==> ((P, \+ R/Test) => Q).
%(mdefault(P=>Q)/nonvar(Q)) ==> (P => mdefault(Q)).
