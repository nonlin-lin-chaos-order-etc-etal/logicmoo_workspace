...
name_template_LF(X,Type2):- name_template_db(X,Type1), type_conversion(Type1,Type2).

name_db([black, sea], black_sea).
name_db([upper, volta], upper_volta).
% WOANT name_db([W1, W2], Name) :- fail, reorder_if_var(W2, atomic_list_concat([W1, '_', W2], Name),
 % TODO   name_template_db(Name, _)).
...
name_template_db(X,Spatial&circle) :-  circle_of_latitude(X), spatial(Spatial).
name_template_db(X,SpatialCity) :- ti(city,X), spatial(Spatial), bfeature_path(Spatial,city,SpatialCity).
name_template_db(X,Spatial&geo&continent) :- spatial(Spatial), ti(continent,X).
name_template_db(X,Spatial&geo&country) :- spatial(Spatial), ti(country,X).
name_template_db(X,Spatial&_) :- spatial(Spatial), ti(region,X).
name_template_db(X,SpatialRiver) :- ti(river,X), spatial(Spatial), bfeature_path(Spatial,river,SpatialRiver).
name_template_db(X,Spatial&geo&seamass) :- spatial(Spatial), ti(seamass,X).
