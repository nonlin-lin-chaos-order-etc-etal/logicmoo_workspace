...
run_pipeline(Text):- 
  once(ignore(text_to_best_tree(Text,format))),
...
run_pipeline(Have, NEEDs, Out):- member(tid=TID, Have),!,run_pipeline_now(TID,Have, NEEDs, Out).
run_pipeline(Have, NEEDs, Out):- gensym(iPipeline, TID),!,run_pipeline_now(TID,[tid=TID|Have], NEEDs, Out).


run_pipeline_now(TID, Have, NEEDs, Out):-
    setup_call_cleanup(
      once((
...
show_pipeline:-
   wdmsg("List of possible data transformations"),
